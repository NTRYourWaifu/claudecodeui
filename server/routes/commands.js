 ...
 */
// translationKey 對應 src/i18n/locales/<lang>/chat.json 內 slashCommands.<key> 區塊
// 若該 key 不存在則 fallback 到下方英文 description
const builtInCommands = [
  {
    name: '/help',
    description: 'Show help documentation for Claude Code',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'help' }
  },
  {
    name: '/clear',
    description: 'Clear the conversation history',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'clear' }
  },
  {
    name: '/compact',
    description: 'Summarize and compress the conversation to free up context',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'compact' }
  },
  {
    name: '/context',
    description: 'Show where the context window is going (tokens used per topic)',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'context' }
  },
  {
    name: '/model',
    description: 'Switch or view the current AI model',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'model' }
  },
  {
    name: '/cost',
    description: 'Display token usage and cost information',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'cost' }
  },
  {
    name: '/usage',
    description: 'Show your subscription usage and rate-limit window',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'usage' }
  },
  {
    name: '/memory',
    description: 'Open CLAUDE.md memory file for editing',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'memory' }
  },
  {
    name: '/init',
    description: 'Analyze the codebase and generate a starter CLAUDE.md',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'init' }
  },
  {
    name: '/review',
    description: 'Review the pending changes on the current branch',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'review' }
  },
  {
    name: '/security-review',
    description: 'Run a security audit on the pending changes',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'securityReview' }
  },
  {
    name: '/config',
    description: 'Open settings and configuration',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'config' }
  },
  {
    name: '/status',
    description: 'Show system status and version information',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'status' }
  },
  {
    name: '/rewind',
    description: 'Rewind the conversation to a previous state',
    namespace: 'builtin',
    metadata: { type: 'builtin', translationKey: 'rewind' }
  }
];
...
`;
  ...
  '/compact': async () => ({
    ...
    data: { prompt: 'Please compact the conversation history: summarize the key decisions and outstanding tasks so far, then I want to continue from that summary.' }
  }),
  '/context': async (args, context) => {
    ...
    return {
      type: 'builtin',
      action: 'info',
      data: { message: `Context: ${used.toLocaleString()} / ${total.toLocaleString()} tokens (${pct}%)` }
    };
  },
  '/usage': async (args, context) => {
    ...
    return {
      type: 'builtin',
      action: 'info',
      data: { message: `Session tokens: ${used.toLocaleString()} / ${total.toLocaleString()} (${pct}%). Subscription rate-limit window is managed by Anthropic; check claude.ai for plan usage.` }
    };
  },
  '/init': async () => ({
    type: 'builtin',
    action: 'inject-prompt',
    data: { prompt: 'Analyze this codebase and create a CLAUDE.md file at the project root with: 1) what this project does, 2) key architecture and folder structure, 3) coding conventions you observe, 4) any non-obvious commands needed to build/test/run. Keep it concise — prefer a one-page reference over long explanation.' }
  }),
  '/review': async () => ({
    type: 'builtin',
    action: 'inject-prompt',
    data: { prompt: 'Review the pending changes on the current branch (uncommitted diff + commits ahead of main). Report: any bugs, security issues, missing tests, inconsistencies with surrounding code style, and any over-engineering. Be specific with file:line references.' }
  }),
  '/security-review': async () => ({
    type: 'builtin',
    action: 'inject-prompt',
    data: { prompt: 'Run a security audit on the pending changes (uncommitted diff + commits ahead of main). Focus on: injection vulnerabilities, auth/permission bypasses, secret exposure, unsafe deserialization, SSRF, path traversal, XSS, and any input that crosses a trust boundary without validation. Report findings with file:line and severity.' }
  })
};
