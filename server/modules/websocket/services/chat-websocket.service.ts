...
type ChatWebSocketDependencies = {
  ...
  spawnGrok: (command: string, options: unknown, writer: WebSocketWriter) => Promise<unknown>;
  ...
  abortGrokSession: (sessionId: string) => boolean;
  resolveToolApproval: (
    requestId: string,
    payload: {
      allow: boolean;
  ...
  isGrokSessionActive: (sessionId: string) => boolean;
...
export function handleChatConnection(
  ...
  ws.on('message', async (rawMessage) => {
    try {
      ...
      if (messageType === 'grok-command') {
        await dependencies.spawnGrok(data.command ?? '', data.options, writer);
        return;
      }
      ...
      if (messageType === 'abort-session') {
        ...
        if (provider === 'cursor') {
          ...
          success = dependencies.abortGrokSession(sessionId);
        } else {
      ...
      if (messageType === 'check-session-status') {
        ...
        if (provider === 'cursor') {
          ...
          isActive = dependencies.isGrokSessionActive(sessionId);
        } else {
      ...
      if (messageType === 'get-active-sessions') {
        writer.send({
          ...
          sessions: {
            ...
            grok: dependencies.getActiveGrokSessions(),
          },
        });
      }
    } catch (error) {
